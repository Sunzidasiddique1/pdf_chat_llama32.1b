...
PAGE_ICON = "✨"
SYSTEM_PROMPT = "You are a friendly AI chatbot having a conversation with a human. Please answer questions based on the uploaded PDF content."
...
def main() -> None:
    ...
    # Check if uploaded_text exists before allowing user to ask questions
    if st.session_state.uploaded_text:
        if prompt := st.chat_input("Type your message here..."):
            st.chat_message("human").write(prompt)

            # Limit the prompt to include only the uploaded text and the user query
            combined_prompt = f"Based on the following text from the PDF:\n\n{st.session_state.uploaded_text}\n\nAnswer the question: {prompt}"

            with st.spinner("Thinking..."):
                config = {"configurable": {"session_id": "any"}}
                response = chain_with_history.invoke({"input": combined_prompt}, config)
                logger.info({"input": combined_prompt}, config)

                # Check if the response is relevant
                if "not found" in response.content.lower() or "irrelevant" in response.content.lower():
                    st.chat_message("ai").write("This is not included in the PDF or URL.")
                else:
